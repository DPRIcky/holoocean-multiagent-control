...
def test_auto_tide(request):
    ...
    with holoocean.environments.HoloOceanEnvironment(
        ...
        assert err < 3000, f"before tide moved this was already wrong"
        ...
        err = mean_square_err(pixels, baseline)
        assert err < 2500, f"this is wrong after the tide moved"
...
def test_tide_collisions():
    ...
    with holoocean.environments.HoloOceanEnvironment(
        ...
        assert abs(locationZ + 2.6) < 0.25, f"our agent did not collide location = {locationZ}"
        env.tide(0, 1)
        for _ in range(100):
            env.tick()
        locationZ = env.tick()["DynamicsSensor"][8]
        assert abs(locationZ) < 0.6, f"our agent did not rise correctly location = {locationZ}"
