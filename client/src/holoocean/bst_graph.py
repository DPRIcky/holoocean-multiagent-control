...
class BST_Visualizer:
    def __init__(self, config=None, biomass_function=None, salinity_function=None, temperature_function=None, agent=None):
        ...
        self.sensor_list = next(agent["sensors"] for agent in self.agent_list if agent["agent_name"] == self.agent)
        self.bst_config = next(sensor["configuration"] for sensor in self.sensor_list if sensor["sensor_type"] == "BSTSensor")
    ...
    def setup_plots(self):
        ...
        try:
            ...
            plt.subplots_adjust(hspace=0.8, wspace=0.4, top=0.85, bottom=0.08, left=0.08, right=0.92)
            ...
            for i in range(3):
                for j in range(2):
                    ...
                    ax.set_title(f'{titles[i]} - {plane} Plane', color='white', fontsize=10, pad=8)
    ...
    def generate_data_xy(self, func):
        ...
        try:
            pos = self.position
            x_range = np.linspace(pos[0] - self.range_val, pos[0] + self.range_val, self.resolution)
            y_range = np.linspace(pos[1] - self.range_val, pos[1] + self.range_val, self.resolution)
            ...
            Z_data = func(location=locations, **bst_sensor["configuration"]).reshape(X.shape)
            return X, Y, Z_data
    ...
    def generate_data_xz(self, func):
        ...
        try:
            pos = self.position
            x_range = np.linspace(pos[0] - self.range_val, pos[0] + self.range_val, self.resolution)
            z_range = np.linspace(pos[2] - self.range_val, pos[2] + self.range_val, self.resolution)
            X, Z = np.meshgrid(x_range, z_range)
            
            # Create location array for all points in the grid
            locations = np.column_stack([
                X.ravel(), 
                np.full(X.size, pos[1]), 
                Z.ravel()
            ])
    ...
    def update_plots(self):
        ...
        try:
                ...
                try:
                    ...
                    for sensor in sensors:
                        if sensor["sensor_type"] == "BSTSensor":
                            ...
                            break
                    # print(bst_sensor)
                    current_value = func(location=self.position, **bst_sensor["configuration"])
                    current_value = float(current_value) if np.isfinite(current_value) else 0.0
                except:
                    current_value = 0.0
                ...
                try:
                    ...
                    if not np.all(Z_data_xy == 0):
                        if self.images[i][0] is None:
                            self.images[i][0] = ax_xy.imshow(Z_data_xy, 
                                                           extent=[X_xy.min(), X_xy.max(), Y_xy.min(), Y_xy.max()],
                                                           origin='lower', cmap=colormap, aspect='equal')
                            self.colorbars[i][0] = plt.colorbar(self.images[i][0], ax=ax_xy, shrink=0.7)
                            self.colorbars[i][0].ax.tick_params(colors='white', labelsize=8)
                            self.colorbars[i][0].set_label(f'{name} ({unit})', color='white', fontsize=8)
                        else:
                            self.images[i][0].set_data(Z_data_xy)
                            self.images[i][0].set_extent([X_xy.min(), X_xy.max(), Y_xy.min(), Y_xy.max()])
                            
                            # Update colorbar limits - adjusted for new biomass model
                            limits = [self.biomass_range, self.salinity_range, self.temperature_range][i]  # biomass, salinity, temperature
                    ...
                    h_line = ax_xy.plot([self.position[0] - crosshair_size, self.position[0] + crosshair_size], 
                                       [self.position[1], self.position[1]], 'w-', linewidth=2, alpha=0.9)[0]
                    v_line = ax_xy.plot([self.position[0], self.position[0]], 
                                       [self.position[1] - crosshair_size, self.position[1] + crosshair_size], 
                                       'w-', linewidth=2, alpha=0.9)[0]
                    center_dot = ax_xy.plot(self.position[0], self.position[1], 'wo', markersize=4, 
                                           markeredgecolor='black', markeredgewidth=1)[0]
                    
                    self.crosshairs[i][0] = [h_line, v_line, center_dot]
                    
                    # Add current value text
                    self.value_texts[i][0] = ax_xy.text(0.02, 0.98, f'{current_value:.2f} {unit}', 
                    ...
                    # Update styling
                    ax_xy.set_xlim(self.position[0] - self.range_val, self.position[0] + self.range_val)
                    ax_xy.set_ylim(self.position[1] - self.range_val, self.position[1] + self.range_val)
                    ax_xy.set_title(f'{name} - XY Plane (Z={self.position[2]:.1f}m)', color='white', fontsize=10, pad=8)
                ...
                try:
                    ...
                    if not np.all(Z_data_xz == 0):
                        if self.images[i][1] is None:
                            self.images[i][1] = ax_xz.imshow(Z_data_xz,
                                                           extent=[X_xz.min(), X_xz.max(), Z_xz.min(), Z_xz.max()],
                                                           origin='lower', cmap=colormap, aspect='equal')
                            self.colorbars[i][1] = plt.colorbar(self.images[i][1], ax=ax_xz, shrink=0.7)
                            self.colorbars[i][1].ax.tick_params(colors='white', labelsize=8)
                            self.colorbars[i][1].set_label(f'{name} ({unit})', color='white', fontsize=8)
                        else:
                            self.images[i][1].set_data(Z_data_xz)
                            self.images[i][1].set_extent([X_xz.min(), X_xz.max(), Z_xz.min(), Z_xz.max()])
                    ...
                    # Create crosshair at vehicle position
                    crosshair_size = self.range_val * 0.1
                    h_line = ax_xz.plot([self.position[0] - crosshair_size, self.position[0] + crosshair_size], 
                                       [self.position[2], self.position[2]], 'w-', linewidth=2, alpha=0.9)[0]
                    v_line = ax_xz.plot([self.position[0], self.position[0]], 
                                       [self.position[2] - crosshair_size, self.position[2] + crosshair_size], 
                                       'w-', linewidth=2, alpha=0.9)[0]
                    center_dot = ax_xz.plot(self.position[0], self.position[2], 'wo', markersize=4, 
                                           markeredgecolor='black', markeredgewidth=1)[0]
                    
                    self.crosshairs[i][1] = [h_line, v_line, center_dot]
                    
                    # Add current value text
                    self.value_texts[i][1] = ax_xz.text(0.02, 0.98, f'{current_value:.2f} {unit}', 
                    ...
                    # Update styling
                    ax_xz.set_xlim(self.position[0] - self.range_val, self.position[0] + self.range_val)
                    ax_xz.set_ylim(self.position[2] - self.range_val, self.position[2] + self.range_val)
                    ax_xz.set_title(f'{name} - XZ Plane (Y={self.position[1]:.1f}m)', color='white', fontsize=10, pad=8)
            ...
            # Update main title
            pos = self.position
            self.fig.suptitle(f'BST Data - Position: ({pos[0]:.1f}, {pos[1]:.1f}, {pos[2]:.1f})', 
