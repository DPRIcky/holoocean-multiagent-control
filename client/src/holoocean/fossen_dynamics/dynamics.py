...
class FossenDynamics:
    """
    Class for handling the connection between a HoloOcean agent and the Fossen model to control the 
    motion of the vehicle.
    """
    
    def __init__(self, vehicle, initial_u_actual=None):
        ...
        # vehicle keeps track of actuator positions
        ...
        self.u_control = self.vehicle.u_actual # Start with no control input
    ...
    def update(self, state):
        ...
        eta, nu = self.convert_state_NWU_to_NED(x) 

        t = state["t"]     

        # Vehicle control systems
        if (self.vehicle.controlMode == 'depthAutopilot'): # NOTE: Not implemented yet
            self.u_control = self.vehicle.depthAutopilot(eta, nu)
        elif (self.vehicle.controlMode == 'headingAutopilot'): # NOTE: Not implemented yet
            self.u_control = self.vehicle.headingAutopilot(eta, nu)   
        elif (self.vehicle.controlMode == 'depthHeadingAutopilot'):
            self.u_control = self.vehicle.depthHeadingAutopilot(eta, nu)             
        elif (self.vehicle.controlMode == 'DPcontrol'): # NOTE: Not implemented yet
            self.u_control = self.vehicle.DPcontrol(eta, nu)                   
        ...
        nu_dot = self.vehicle.dynamics(eta, nu, self.u_control)
        
        # NOTE: we just compute accels, UE handles the integration and state update.

        # Return the acceleration in world frame for holoocean
        return self.convert_accel_NED_to_NWU(x, nu_dot)
    ...
    def convert_state_NWU_to_NED(self, x):
        '''
    ...
    def convert_accel_NED_to_NWU(self, x, accel):
        '''
