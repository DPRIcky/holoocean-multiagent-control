...
void UHoveringAUVControlPD::Execute(void* const CommandArray, void* const InputCommand, float DeltaSeconds) {
	...
	LinAccel[0] = FMath::Clamp(LinAccel[0], -AUV_CONTROL_MAX_LIN_ACCEL, AUV_CONTROL_MAX_LIN_ACCEL);
	LinAccel[1] = PositionController_Y.ComputePIDDirect(DesiredPosition[1], Position[1], LinearVelocity[1], DeltaSeconds);
	LinAccel[1] = FMath::Clamp(LinAccel[1], -AUV_CONTROL_MAX_LIN_ACCEL, AUV_CONTROL_MAX_LIN_ACCEL);
	LinAccel[2] = PositionController_Z.ComputePIDDirect(DesiredPosition[2], Position[2], LinearVelocity[2], DeltaSeconds);
	LinAccel[2] = FMath::Clamp(LinAccel[2], -AUV_CONTROL_MAX_LIN_ACCEL, AUV_CONTROL_MAX_LIN_ACCEL);
	...
	AngAccel[0] = FMath::Clamp(AngAccel[0], -AUV_CONTROL_MAX_ANG_ACCEL, AUV_CONTROL_MAX_ANG_ACCEL);
	...
	AngAccel[1] = FMath::Clamp(AngAccel[1], -AUV_CONTROL_MAX_ANG_ACCEL, AUV_CONTROL_MAX_ANG_ACCEL);
	...
	AngAccel[2] = FMath::Clamp(AngAccel[2], -AUV_CONTROL_MAX_ANG_ACCEL, AUV_CONTROL_MAX_ANG_ACCEL);
	...
	FVector tau = HoveringAUV->Volume * HoveringAUV->WaterDensity * 9.81 * FVector::CrossProduct(e3, COB);
