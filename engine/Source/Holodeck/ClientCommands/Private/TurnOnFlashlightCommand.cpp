...
void UTurnOnFlashlightCommand::Execute() {
	...
	if (StringParams.size() != 1 || NumberParams.size() != 10) {
		UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UTurnOnFlashlightCommand. Command not executed."));
		return;
	}

	AHolodeckGameMode* GameTarget = static_cast<AHolodeckGameMode*>(Target);
	if (GameTarget == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. UTurnOnFlashlightCommand::Flashlight not turned on."));
		return;
	}

	UWorld* World = Target->GetWorld();
	if (World == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UTurnOnFlashlightCommand::Execute found world as nullptr."));
		return;
	}
    ...
    FRotator lightAngle = FRotator(FMath::Clamp(NumberParams[5], -70.0f, 70.0f), FMath::Clamp(NumberParams[6], -70.0f, 70.0f), 0);
    FLinearColor lightColor = FLinearColor(FMath::Clamp(NumberParams[7], 0.0f, 1.0f), FMath::Clamp(NumberParams[8], 0.0f, 1.0f), FMath::Clamp(NumberParams[9], 0.0f, 1.0f));
    ...
    if (FlashlightName.Equals("flashlight1")) {
        ...
        UE_LOG(LogHolodeck, Error, TEXT("Unknown flashlight name: %s"), *FlashlightName);
        return;
    }
    ...
    Vehicle->CallFunctionByNameWithArguments(TEXT("ApplyFlashlightSettingsFromManager"), *GLog, nullptr, true);
}
}