...
void UWaterFogCommand::Execute() {
    ...
    if (NumberParams.size() != 5) {
        UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UWaterFogCommand. Command not executed."));
        return;
    }
    AHolodeckGameMode* GameTarget = static_cast<AHolodeckGameMode*>(Target);
    if (GameTarget == nullptr) {
        UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. UWaterFogCommand::Fog not changed."));
        return;
    }
    UWorld* World = Target->GetWorld();
    if (World == nullptr) {
        UE_LOG(LogHolodeck, Warning, TEXT("UWaterFogCommand::Execute found world as nullptr. Fog not changed."));
        return;
    }

    // All Post Process Volumes
    TArray<AActor*> FoundPPVolumes;
    UGameplayStatics::GetAllActorsOfClass(World, APostProcessVolume::StaticClass(), FoundPPVolumes);
    if (FoundPPVolumes.Num() == 0) {
        UE_LOG(LogHolodeck, Warning, TEXT("No Post Process Volumes found in the scene. Fog not changed."));
        return;
    }

    // Read arguments
    float fogDensity = FMath::Clamp(NumberParams[0], 0.0f, 10.0f);
    float fogDepth = (FMath::Clamp(NumberParams[1], 0.0f, 10.0f)) * 40000; //depth is in the range of 40k to 400k, but user inputs 1 to 10
    ...
    for (int32 i = 0; i < Blendables.Num(); ++i) {
        ...
        } else if (UMaterialInstanceDynamic* DynMat = Cast<UMaterialInstanceDynamic>(Obj)) {
