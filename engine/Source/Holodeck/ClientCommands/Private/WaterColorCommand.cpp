...
void UWaterColorCommand::Execute() {
    ...
    if (NumberParams.size() != 3) {
        UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UWaterColorCommand. Command not executed."));
        return;
    }
    ...
    if (World == nullptr) {
        UE_LOG(LogHolodeck, Warning, TEXT("UAddSensorCommand::Execute found world as nullptr."));
        return;
    }
    ...
    for (AActor* actor : actors) {
        ...
        tint.A = 1;  // this value doesn't affect the water color -- but still needs to be set
