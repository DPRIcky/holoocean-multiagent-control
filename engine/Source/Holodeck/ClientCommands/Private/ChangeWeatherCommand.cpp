...
void UChangeWeatherCommand::Execute() {
	...
	if (NumberParams.size() != 1) {
		UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UChangeWeatherCommand. Command not executed."));
		return;
	}

	AHolodeckGameMode* GameTarget = static_cast<AHolodeckGameMode*>(Target);
	if (GameTarget == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. UChangeWeatherCommand::Weather not changed."));
		return;
	}

	UWorld* World = Target->GetWorld();
	if (World == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UChangeWeatherCommand::Execute found world as nullptr. Weather not changed."));
		return;
	}
    ...
    UGameplayStatics::GetAllActorsOfClass(World, AVolumetricCloud::StaticClass(), VolumetricClouds);
    TArray<AActor*> Planes;
	UGameplayStatics::GetAllActorsOfClass(World, AStaticMeshActor::StaticClass(), Planes);
    
    // Checking actors exist
    if (DirectionalLights.Num() == 0 || SkyLights.Num() == 0 || HeightFogs.Num() == 0 || SkyAtmospheres.Num() == 0 || VolumetricClouds.Num() == 0 || Planes.Num() == 0 ) {
        UE_LOG(LogHolodeck, Warning, TEXT("Not enough actors found in the scene. Weather not changed."));
        return;
    }
    ...
    if (weather < 0 || weather > 2) {
        UE_LOG(LogHolodeck, Warning, TEXT("Invalid weather option: %d.  (0 - sunny, 1 - cloudy, 2 - rainy)."), weather);
        return;
    }
    ...
    if (!SunnyClouds || !CloudyClouds) {
        UE_LOG(LogHolodeck, Warning, TEXT("Could not find both Sunny and Cloudy VolumetricCloud actors."));
        return;
    }
    ...
    USkyAtmosphereComponent* SkyAtmosphereComponent = Cast<USkyAtmosphereComponent>(SkyAtmosphere->GetComponentByClass(USkyAtmosphereComponent::StaticClass()));

    if (!DirectionalLightComponent || !SkyLightComponent || !HeightFogComponent || !SkyAtmosphereComponent) {
        UE_LOG(LogHolodeck, Warning, TEXT("One or more weather components are null. Weather not changed."));
        return;
    }
            ...
            if (MeshActor){
                ...
                if (MeshComp){
                    ...
                    if (PreviousMaterial.Contains("DamFlat")){
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/Worlds/WaterMaterial/MM_Ocean_DamFlat.MM_Ocean_DamFlat"));
                        //Check new material
                        if (NewMaterial){
                            // Apply new material
                            ...
                            UE_LOG(LogHolodeck, Warning, TEXT("New material not found"));
                        }
                    }
                    else if (PreviousMaterial.Contains("OpenWater")){
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/Worlds/WaterMaterial/MM_Ocean_OpenWater.MM_Ocean_OpenWater"));
                        if (NewMaterial){
                            // Apply new material
                            MeshComp->SetMaterial(0, NewMaterial);
                        }
                        ...
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/Worlds/WaterMaterial/MM_Ocean_Simple.MM_Ocean_Simple"));
                        if (NewMaterial){
                            // Apply new material
                            MeshComp->SetMaterial(0, NewMaterial);
                        }
                        ...
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/StarterContent/Materials/M_TranslucentBlue_Water.M_TranslucentBlue_Water"));
                        if (NewMaterial){
                            // Apply new material
                            MeshComp->SetMaterial(0, NewMaterial);
                        }
        ...
        for (AActor* WaterPlane : WaterPlanes){
            ...
            if (MeshActor){
                ...
                if (MeshComp){
                    ...
                    // Get new material
                    if (PreviousMaterial.Contains("DamFlat")){
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/WeatherContent/WaterRipples/MM_Ocean_Ripples_DamFlat.MM_Ocean_Ripples_DamFlat"));
                        //Check new material
                        if (NewMaterial){
                            // Apply new material
                            MeshComp->SetMaterial(0, NewMaterial);
                        ...
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/WeatherContent/WaterRipples/MM_Ocean_Ripples_OpenWater.MM_Ocean_Ripples_OpenWater"));
                        if (NewMaterial){
                            // Apply new material
                            MeshComp->SetMaterial(0, NewMaterial);
                        }
                        ...
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/WeatherContent/WaterRipples/MM_Ocean_Ripples_Simple.MM_Ocean_Ripples_Simple"));
                        if (NewMaterial){
                            // Apply new material
                            MeshComp->SetMaterial(0, NewMaterial);
                        }
                        ...
                        UMaterialInterface* NewMaterial = LoadObject<UMaterialInterface>(nullptr, TEXT("/Game/WeatherContent/WaterRipples/MM_Ocean_Ripples_Test.MM_Ocean_Ripples_Test"));
                        if (NewMaterial){
                            // Apply new material
                            MeshComp->SetMaterial(0, NewMaterial);
                        }
