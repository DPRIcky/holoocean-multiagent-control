...
void USetRainParametersCommand::Execute() {
	...
	if (NumberParams.size() != 4) {
		UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in USetRainParametersCommand. Command not executed."));
		return;
	}

	AHolodeckGameMode* GameTarget = static_cast<AHolodeckGameMode*>(Target);
	if (GameTarget == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. USetRainParametersCommand::Rain not modified."));
		return;
	}

	UWorld* World = Target->GetWorld();
	if (World == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("USetRainParametersCommand::Execute found world as nullptr. Rain not modified."));
		return;
	}

    FVector newRainVelocity = FVector(
        NumberParams[0],
        NumberParams[1],
        NumberParams[2]
    );
