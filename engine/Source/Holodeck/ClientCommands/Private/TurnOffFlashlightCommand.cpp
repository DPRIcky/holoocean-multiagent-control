...
void UTurnOffFlashlightCommand::Execute() {
	...
	if (StringParams.size() != 1 || NumberParams.size() != 0) {
		UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UTurnOffFlashlightCommand. Command not executed."));
		return;
	}

	AHolodeckGameMode* GameTarget = static_cast<AHolodeckGameMode*>(Target);
	if (GameTarget == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. UTurnOffFlashlightCommand::Flashlight not turned off."));
		return;
	}

	UWorld* World = Target->GetWorld();
	if (World == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UTurnOffFlashlightCommand::Execute found world as nullptr."));
		return;
	}
