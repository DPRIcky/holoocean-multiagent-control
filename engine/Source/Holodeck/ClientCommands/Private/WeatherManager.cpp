...
{
    bIsRainy = bRain;
    UE_LOG(LogTemp, Log, TEXT("WeatherManager: bIsRainy set to %s"), bRain ? TEXT("true") : TEXT("false"));
}

void AWeatherManager::SetRainVelocity(FVector newRainVelocity)
{
    rainVelocity = newRainVelocity;
    UE_LOG(LogTemp, Log, TEXT("WeatherManager: rainVelocity set to %s"), *rainVelocity.ToString());}
