...
void UAirFogCommand::Execute() {
    ...
    if (NumberParams.size() != 5) {
        UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UAirFogCommand. Command not executed."));
        return;
    }
    AHolodeckGameMode* GameTarget = static_cast<AHolodeckGameMode*>(Target);
    if (GameTarget == nullptr) {
        UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. UAirFogCommand::Fog not changed."));
        return;
    }
    UWorld* World = Target->GetWorld();
    if (World == nullptr) {
        UE_LOG(LogHolodeck, Warning, TEXT("UAirFogCommand::Execute found world as nullptr. Fog not changed."));
        return;
    }
