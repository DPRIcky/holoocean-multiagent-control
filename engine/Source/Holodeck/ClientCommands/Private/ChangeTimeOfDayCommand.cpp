...
void UChangeTimeOfDayCommand::Execute() {
	...
	if (NumberParams.size() != 1) {
		UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UChangeTimeOfDayCommand. Command not executed."));
		return;
	}
	...
	if (GameTarget == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*. UChangeTimeOfDayCommand::Time of day not changed."));
		return;
	}

	UWorld* World = Target->GetWorld();
	if (World == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UChangeTimeOfDayCommand::Execute found world as nullptr. Time of day not changed."));
		return;
	}
    ...
    if (DirectionalLights.Num() == 0) {
        UE_LOG(LogHolodeck, Warning, TEXT("No Directional Lights found in the scene. Time of day not changed."));
        return;
    }
    ...
    if (TimeOfDay < 0 || TimeOfDay > 24) {
        UE_LOG(LogHolodeck, Warning, TEXT("Invalid time of day: %f. Must be in [0, 24]."), TimeOfDay);
        return;
    }
	...
	for (int32 j = 0; j < DirectionalLights.Num(); j++) {
        ...
        UDirectionalLightComponent* LightComponent = Cast<UDirectionalLightComponent>(BaseLightComponent);
