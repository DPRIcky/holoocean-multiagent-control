...
void UTideCommand::Execute() {
	...
	if (NumberParams.size() != 2) {
		UE_LOG(LogHolodeck, Error, TEXT("Unexpected argument length found in UTideCommand. Command not executed."));
		return;
	}
	...
	if (GameTarget == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UCommand::Target is not a UHolodeckGameMode*."));
		return;
	}

	UWorld* World = Target->GetWorld();
	if (World == nullptr) {
		UE_LOG(LogHolodeck, Warning, TEXT("UTideCommand::Execute found world as nullptr."));
		return;
	}

	TArray<AActor*> found_agents; // this gets our robots
	UGameplayStatics::GetAllActorsOfClass(World, AHolodeckAgent::StaticClass(), found_agents); 

	TArray<AActor*> found_volumes; //this gets our post process volume
	UGameplayStatics::GetAllActorsOfClass(World, APostProcessVolume::StaticClass(), found_volumes);
	...
	for (AActor* actor : found_actors) {
		...
		// else if (actor->GetActorLabel() == "PhysicsVolume") { -- this makes it really laggy -- also getactorlabel doesn't work with packaging -- have to either have a specific type or gotta have tags
		// 	found_volumes.Add(actor);
		// }
	}
	...
	for (AActor* actor : found_floats) {
		...
		//if (hit.IsValidBlockingHit()) {} -- could be useful if want to do something when the object hit the ground eg. explode
	...
	for (int32 i = 0; i < found_volumes.Num(); i++) {
		...
		// UE_LOG(LogHolodeck, Log, TEXT("Post Process volume befroe tide: %f, %f, %f"), volume_location.X, volume_location.Y, volume_location.Z);
		if (!NumberParams[1]) {
			volume_location.Z += adjustment;
			found_volumes[i]->SetActorLocation(volume_location);
		} else {
			if (found_volumes[i]->Tags.Contains("WaterPPV")) {
				// UE_LOG(LogHolodeck, Log, TEXT("Initial ppv, %f,%f,%f"), PPVUnderwater.X, PPVUnderwater.Y, PPVUnderwater.Z);
				FVector WaterLocation(PPVUnderwater.X, PPVUnderwater.Y, PPVUnderwater.Z + adjustment);
				// UE_LOG(LogHolodeck, Log, TEXT("final ppv, %f,%f,%f"), PPVUnderwater.X, PPVUnderwater.Y, PPVUnderwater.Z);
				// UE_LOG(LogHolodeck, Log, TEXT("Initial water, %f,%f,%f"), found_volumes[i]->GetActorLocation().X, found_volumes[i]->GetActorLocation().Y, found_volumes[i]->GetActorLocation().Z);
				found_volumes[i]->SetActorLocation(WaterLocation);
				// UE_LOG(LogHolodeck, Log, TEXT("Final water, %f,%f,%f"), found_volumes[i]->GetActorLocation().X, found_volumes[i]->GetActorLocation().Y, found_volumes[i]->GetActorLocation().Z);
		...
		// UE_LOG(LogHolodeck, Log, TEXT("Post Process volume after tide: %f, %f, %f"), volume_location.X, volume_location.Y, volume_location.Z);
	}

	//moving the agents
	for (int32 i = 0; i < found_agents.Num(); i++) {
		AHolodeckBuoyantAgent* BuoyantAgent = Cast<AHolodeckBuoyantAgent>(found_agents[i]);
		if (i == 0) {
			// UE_LOG(LogHolodeck, Log, TEXT("adjustment %f, surface level %f"), adjustment, BuoyantAgent->SurfaceLevel);
			// UE_LOG(LogHolodeck, Log, TEXT("new surface level %f"), BuoyantAgent->SurfaceLevel);
		}
		if (!NumberParams[1]) {
